...
incoming jobs through a **headless coding agent** — streaming partial results and
a final outcome back over the same socket. It reconnects with exponential backoff
and re-registers when the socket drops.

## Backends

Each box drives jobs through one backend, selected with `MUSTER_AGENT_BACKEND`:

| Backend | Value | Invocation | Output |
| --- | --- | --- | --- |
| Claude Code | `claude` (default) | `claude -p <job> --output-format stream-json …` | stream-json; assistant text extracted per frame |
| OpenCode | `opencode` | `opencode run <job>` | plain text; each stdout line streamed as a chunk |

Both look identical to the control plane — only the local `AgentRunner` differs.
`build_runner(settings)` constructs the right one from config.

## Design

Every external boundary is injected, so the whole register/heartbeat/job loop is
exercised with **no network, no `claude`/`opencode` binary, and no GPU**:

| Boundary | Production | Tests |
| --- | --- | --- |
| Control-plane socket | `connect_websocket` (`websockets`) | `FakeTransport` |
| Job execution | `AgentRunner` (claude/opencode subprocess) | `FakeRunner` |
...
export MUSTER_AGENT_BACKEND=opencode          # claude (default) | opencode
